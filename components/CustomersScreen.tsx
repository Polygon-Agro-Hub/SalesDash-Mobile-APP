...
//   useEffect(() => {
//     // Introduce a delay before fetching customers
...
const CustomersScreen: React.FC<CustomersScreenProps> = ({ navigation }) => {
  ...
  //   if (query === "") {
  //     setFilteredCustomers(customers); // If the search query is empty, show all customers
  //   } else {
  ...
  //     );
  //     setFilteredCustomers(filteredData); // Set the filtered customers
  //   }
  // };
  const handleSearch = (query: string) => {
    setSearchQuery(query);
  
    // Format the search query if it's a phone number
